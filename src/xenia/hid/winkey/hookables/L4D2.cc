...
L4D2Game::~L4D2Game() = default;

bool L4D2Game::IsGameSupported() {
  return kernel_state()->title_id() == kTitleIdL4D2;
}

#define IS_KEY_DOWN(x) (input_state.key_states[x])

bool L4D2Game::DoHooks(uint32_t user_index, RawInputState& input_state,
  ...
  bool result = kernel_state()->processor()->Execute(
      current_thread->thread_state(), 0x8697DB30);
  ...
  xe::be<uint32_t>* angle_offset =
      kernel_memory()->TranslateVirtual<xe::be<uint32_t>*>(player_ptr + 0x4A94);
...
bool L4D2Game::ModifierKeyHandler(uint32_t user_index,
