...
OrangeBoxGame::OrangeBoxGame() {
  original_sensitivity = cvars::sensitivity;
  engine_360 = NULL;
};

OrangeBoxGame::~OrangeBoxGame() = default;

bool OrangeBoxGame::IsGameSupported() {
  return kernel_state()->title_id() == kTitleIdOrangeBox;
}

#define IS_KEY_DOWN(x) (input_state.key_states[x])

bool OrangeBoxGame::DoHooks(uint32_t user_index, RawInputState& input_state,
  ...
  xe::be<uint32_t>* angle_offset =
      kernel_memory()->TranslateVirtual<xe::be<uint32_t>*>(0x863F53A8);
...
bool OrangeBoxGame::ModifierKeyHandler(uint32_t user_index,
