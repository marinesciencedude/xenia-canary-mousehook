...
L4D1Game::~L4D1Game() = default;

bool L4D1Game::IsGameSupported() {
  return kernel_state()->title_id() == kTitleIdL4D1;
}

#define IS_KEY_DOWN(x) (input_state.key_states[x])

bool L4D1Game::DoHooks(uint32_t user_index, RawInputState& input_state,
  ...
  bool result = kernel_state()->processor()->Execute(
      current_thread->thread_state(), 0x86536888);
  ...
  xe::be<uint32_t>* angle_offset =
      kernel_memory()->TranslateVirtual<xe::be<uint32_t>*>(player_ptr + 0x4B44);
...
bool L4D1Game::ModifierKeyHandler(uint32_t user_index,
