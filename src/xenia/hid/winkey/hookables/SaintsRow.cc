...
const long long hold_time = static_cast<long long>(cvars::right_stick_hold_time_workaround);
...
std::string SaintsRowGame::ChooseBinds() {
  ...
  if (wheel_status && *wheel_status != 0 && *menu_status == 2) { // Need to check menu_status otherwise pressing B in some menus will cause you to get stuck if WheelOpen binds difer from Menu.
    return "WheelOpen";
  }
...
bool SaintsRowGame::ModifierKeyHandler(uint32_t user_index,
  ...
  if (thumb_lx != 0 || thumb_ly != 0) { // Required otherwise stick is pushed to the right by default.
